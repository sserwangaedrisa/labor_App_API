          ...
          <table className="w-full">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="text-left px-6 py-4">
                  <span className="text-sm font-semibold text-foreground">Assigned Site</span>
                </th>
                <th className="text-center px-6 py-4">
            ...
            <tbody>
              {filteredUsers?.map((user) => (
                <tr key={user?.id} className="border-b border-border hover:bg-muted/30 transition-smooth">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0">
                        <Icon key='color' name={getRoleIcon(user?.role)} size={18} color="var(--color-primary)" />
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-muted-foreground">{user?.assignedSite || 'Not assigned'}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex justify-center">
            ...
            <div key={user?.id} className="p-4">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0">
                    <Icon name={getRoleIcon(user?.role)} size={18} color="var(--color-primary)" />
                  </div>
              ...
              <div className="space-y-2 mb-3">
                <div className="flex items-center gap-2 text-sm">
                  <Icon key='mail' name="Mail" size={14} className="text-muted-foreground flex-shrink-0" />
                  <span className="text-foreground truncate">{user?.email}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Icon key="phone" name="Phone" size={14} className="text-muted-foreground flex-shrink-0" />
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium text-foreground">{user?.assignedSite || 'Not assigned'}</span>
                </div>
              </div>
        ...
        {filteredUsers?.length === 0 && (
          <div className="p-8 md:p-12 text-center">
            <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center mx-auto mb-4">
              <Icon key='users' name="Users" size={32} color="var(--color-muted-foreground)" />
            </div>
